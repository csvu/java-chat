...
public class UserRelationController {
    ...
    private void initializeActivityTable() {
        ...
        TableColumn<UserLoginDTO, Timestamp> loginDateCol = new TableColumn<>("Open At");
    ...
    private void loadActivities() {
        Task<List<Object[]>> task = new Task<>() {
            ...
            protected List<Object[]> call() throws Exception {
                return userManagementDAO.getOpenTimeByUserId(userId);
            }
        };

        task.setOnSucceeded(e -> {
            List<Object[]> openTime = task.getValue();
            List<UserLoginDTO> activities = openTime.stream()
                .map(data -> {
                    UserDTO userDTO = (UserDTO) data[0];
                    OpenTimeDTO openTimeDTO = (OpenTimeDTO) data[1];
                    return new UserLoginDTO(userDTO.getUsername(), userDTO.getEmail(), userDTO.getDisplayName(),
                        openTimeDTO.getOpenAt());
                })
